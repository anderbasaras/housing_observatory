...
class PeticionAnomalia(BaseModel):
    """Datos de un anuncio para auditar si su precio es anomalo.

    Igual que la peticion de precio, pero ADEMAS incluye monthly_price:
    aqui el precio es una entrada (lo que pide el anuncio), no la salida.
    Sin el precio no se pueden calcular tres de las cuatro senales.
    """
    monthly_price: float = Field(..., gt=0, description="Precio mensual publicado en euros")
    area_sqm: float = Field(..., gt=0, description="Superficie en m2")
    n_rooms: int = Field(..., ge=1, description="Numero de habitaciones (minimo 1)")
    typology_grouped: str = Field(..., description="flat, penthouse u other")
    zona_modelo: str = Field(..., description="Zona en formato Municipio|Barrio")
...
class DesgloseSenales(BaseModel):
    """Contribucion de cada una de las cuatro senales."""
    reglas: float = Field(..., description="Proporcion de reglas de consistencia activadas (0-1)")
    zona: float = Field(..., description="Componente normalizado de desviacion de zona (0-1)")
    modelo: float = Field(..., description="Componente normalizado de desviacion del modelo (0-1)")
    iso: float = Field(..., description="Componente normalizado del Isolation Forest (0-1)")


class RespuestaAnomalia(BaseModel):
    """Respuesta del endpoint de anomalia."""
    anomaly_score: float = Field(..., description="Puntuacion compuesta de anomalia (0-1)")
    precio_esperado_eur: float = Field(..., description="Precio que estima el modelo, en euros")
    z_modelo: float = Field(..., description="Desviaciones tipicas frente a lo esperado")
    z_zona: float = Field(..., description="Desviaciones tipicas frente a la mediana de zona")
    n_reglas_activadas: int = Field(..., description="Cuantas reglas de consistencia se activan")
    desglose: DesgloseSenales
    aviso: str = Field(..., description="Indicio estadistico, no prueba de fraude")
